...
if (!existsSync(electronBin)) {
  console.warn(
    "⚠️  Electron binary not found in local node_modules. Continuing, assuming it is available on PATH."
  );
}
...
if (!existsSync(mainPath)) {
  ...
  console.error(
    '   Ensure you have created electron/main.cjs and package.json "main" points to it.'
  );
  process.exit(1);
}
...
viteProcess.stdout.on("data", (data) => {
  ...
  if (
    ...
    (output.includes("5173") || output.match(/http:\/\/localhost:\d+/))
  ) {
    viteReady = true;
